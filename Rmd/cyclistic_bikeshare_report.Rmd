...
output:
  pdf_document:
    includes:
      in_header: "../resources/latex/preamble.tex"
    toc: true
    toc_depth: 3
  html_document:
    includes:
...
1. Increase maximum ride duration for members during off-peak times: This could encourage longer rides, enhance user experience, and potentially be an impetus for casual riders to become annual members.     
...
table_format <- ifelse(knitr::is_html_output(), "html", "latex")
custom_table_theme(table_data, table_format)
```
...
table_data <- additional_variables %>%
  kable(
...
The ride distance plot illustrates that casual riders tend to travel longer distances than annual members, with an average ride distance of approximately `r comma(round(rider_stats$avg_ride_distance[1], 0))` meters (`r round(conv_unit(rider_stats$avg_ride_distance[1], "m", "mi"), 2)` miles) on weekdays and `r comma(round(rider_stats$avg_ride_distance[3], 0))` meters (`r round(conv_unit(rider_stats$avg_ride_distance[3], "m", "mi"), 2)` miles)  on weekends. In contrast, annual members have slightly shorter average ride distances, with about `r round(rider_stats$avg_ride_distance[2], 0)` meters (`r round(conv_unit(rider_stats$avg_ride_distance[2], "m", "mi"), 2)` miles) on weekdays and `r comma(round(rider_stats$avg_ride_distance[4], 0))` meters (`r round(conv_unit(rider_stats$avg_ride_distance[4], "m", "mi"), 2)` miles) on weekends. This pattern suggests that casual riders consistently travel longer distances than annual members, indicating a preference for more extended rides, which might reflect their more leisure-based use of the bike-share system and a possible preference for longer exploratory trips.     
...
1. Increase maximum ride duration for members during off-peak times:   
  ...
  - The current maximum ride duration for members is 45 minutes, compared to 180 minutes for casual riders. Given the difference in the way the two groups appear to use the system, increasing the maximum ride duration limit for members during non-peak hours could encourage longer rides and accommodate leisurely trips. This adjustment could enhance the user experience for current members and potentially lead to the conversion of casual riders to members. By offering more flexibility in ride duration, Cyclistic can cater to the diverse needs of riders and encourage casual riders to consider becoming annual members.     
