...
The executive summary provides a concise overview of the analysis conducted on Cyclistic's bike-share program data, focusing on the usage patterns of annual members and casual riders. The analysis aimed to uncover insights into how these two rider segments utilize the bike-share system differently, with the ultimate goal of informing targeted marketing strategies to convert casual riders into annual members. Here are the key findings and recommendations derived from the analysis:
...
```{r raw_data_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("resources", "images", "raw_data.png"))
```
...
```{r structure_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("resources", "images", "glimpse_raw.png"))
```


```{r structure_of_data, warning=FALSE, message=FALSE, echo=FALSE}
# if (knitr::is_html_output()) {
#   knitr::include_graphics(here::here("resources", "images", "glimpse_raw.png"))
# } else {
#   knitr::include_graphics(here::here("resources", "images", "glimpse_raw.png"), out.width='0.8\\textwidth')
# }
```
...
These transformations enhance the dataset and prepares it for in-depth exploratory analyses, ensuring that the data is well-suited for comprehensive and advanced analytical tasks.
...
To get a general understanding of the patterns of Cyclistic's bike-share users, we first examined the proportion of rides for both annual members and casual riders. 
...
```{r ridership_pie_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "ridership_ridertype_pie.png"))
```

```{r ridership_pie, warning=FALSE, message=FALSE, echo=FALSE}
# if (knitr::is_html_output()) {
#   knitr::include_graphics(here::here("output", "plots", "ridership_ridertype_pie.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "ridership_ridertype_pie.png"), out.width='0.8\\textwidth')
# }
```

The pie chart illustrates that annual members accounted for the majority of rides, representing 66% of the total rides, while casual riders made up the remaining 34%. This distribution highlights the significant dominance of annual members in utilizing the bike-share system, but does not provide insights into how these riders differ in their usage patterns.
...
```{r monthly_ridership_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "monthly_ridership.png"))
```
...
#   knitr::include_graphics(here::here("output", "plots", "monthly_ridership.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "monthly_ridership.png"), out.width='0.8\\textwidth')
# }
```

The bar plot shows that from April 2023 to March 2024, members consistently outnumber casual riders, though the proportions vary by month. Throughout most of the warmer months, from April to October, there is a significant presence of casual riders (visualized in purple), suggesting a higher activity level or preference for the bike-share system among this group during favorable weather conditions. Casual riders often accounted for 32% to 40% of the total rides during these months.

Conversely, from November to February, there is a noticeable decline in the proportion of rides taken by casual riders, indicated by a shrinking purple section in the bars. Casual riders only accounted for 16% to 26% of the total rides during these months. This change highlights a substantial seasonal effect, with casual rider usage visibly reducing during the colder months.
...
With these calculations, we identified that the mode for Cyclistic members is Thursday, and for casual riders, it is Saturday. This information guided us to visualize and compare the average daily ridership for members and casual riders throughout the week.     
...
```{r daily_ridership_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "daily_ridership.png"))
```
...
#   knitr::include_graphics(here::here("output", "plots", "daily_ridership.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "daily_ridership.png"), out.width='0.8\\textwidth')
# }
```

Comparative Analysis:

The bar plot presents the average daily rides for casual riders (in purple) and members (in teal) across each day of the week, from Sunday to Saturday. The data reveals distinct usage patterns between the two groups.
...
```{r hourly_ridership_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "hourly_ridership_heatmap.png"))
```
...
#   knitr::include_graphics(here::here("output", "plots", "hourly_ridership_heatmap.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "hourly_ridership_heatmap.png"), out.width='0.8\\textwidth')
# }
```
...
We next wanted to examine how ride distances and durations vary between Cyclistic's casual and member riders. By comparing the average ride distances and durations for each rider type, we aimed to uncover insights into the typical ride lengths and durations preferred by casual and member riders.     
...
```{r distance_duration_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "mean_distance_duration.png"))
```

```{r distance_duration_plot, warning=FALSE, message=FALSE, echo=FALSE}
# if (knitr::is_html_output()) {
#   knitr::include_graphics(here::here("output", "plots", "mean_distance_duration.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "mean_distance_duration.png"), out.width='0.8\\textwidth')
# }
```
...
Casual Riders (Purple): On weekdays, the average ride distance for casual riders is about 2,140 meters, which increases to approximately 2,290 meters on weekends. This indicates that casual riders are likely to engage in longer rides during weekends, possibly for leisure or exploration.
Members (Teal): Members show a slightly lower average ride distance than casual riders, with about 2,086 meters on weekdays and an increase to roughly 2,182 meters on weekends. This pattern suggests that members also extend their ride distances on weekends, though the increase is less pronounced compared to casual riders.
Comparative Analysis:

Both casual riders and members tend to ride longer distances on weekends compared to weekdays, indicating a general trend of more extended leisure activities during weekends.
Casual riders consistently travel longer distances than members on both weekdays and weekends, which might reflect their less routine-based use of the bike-share system and a possible preference for longer exploratory trips.
...
Casual riders consistently travel longer distances than member riders, regardless of whether it’s a weekday or weekend. This could suggest that casual riders are more likely to use the service for longer trips or exploratory rides.
Both rider types increase their average ride distances on weekends compared to weekdays. This could be due to more free time available or different trip purposes on weekends (e.g., leisure, sightseeing) compared to weekdays (e.g., commuting).
The difference in average ride distance between casual and member riders is greater on weekends (111 m) than on weekdays (134 m). This might indicate that the usage patterns of casual riders vary more between weekdays and weekends compared to member riders.
Average Ride Duration:
Casual riders spend significantly more time riding than member riders. On weekdays, casual riders spend about 42% more time riding than members (15.6 minutes vs. 11 minutes). This difference is even more pronounced on weekends, with casual riders spending about 56% more time riding than members (19.5 minutes vs. 12.5 minutes).
Both rider types spend more time riding on weekends compared to weekdays, which could be due to more leisure time available.
The increase in ride duration from weekdays to weekends is more pronounced for casual riders (25% increase) compared to member riders (14% increase). This could suggest that casual riders’ usage of the service is more influenced by the day of the week compared to member riders.
Implications:
These patterns could have various implications for service operation and marketing. For example, promotional campaigns or special offers could be targeted towards casual riders on weekends when they are more likely to take longer and more frequent rides.
Understanding these patterns could also help with resource allocation. For instance, knowing that rides are likely to be longer for all users on weekends could inform decisions about bike availability and distribution.
...
```{r bike_preference_pdf, warning=FALSE, message=FALSE, echo=FALSE, eval=!knitr::is_html_output(), out.width="0.8\\textwidth"}
knitr::include_graphics(here::here("output", "plots", "bike_preference.png"))
```

```{r bike_type_preferences_plot, warning=FALSE, message=FALSE, echo=FALSE}
# if (knitr::is_html_output()) {
#   knitr::include_graphics(here::here("output", "plots", "bike_preference.png"))
# } else {
#   knitr::include_graphics(here::here("output", "plots", "bike_preference.png"), out.width='0.8\\textwidth')
# }
```
...
Casual Riders: Show a notable preference for electric bikes, with 55% opting for electric bikes compared to 45% for classic bikes. This indicates that casual riders favor the convenience and ease of use that electric bikes offer.
Members: Display a more balanced preference between bike types, slightly favoring classic bikes (51%) over electric bikes (49%). This suggests that members are comfortable using both types of bikes for their varied needs, including commuting and leisure.
Implications:

Casual Riders: The preference for electric bikes among casual riders suggests they value the ease and reduced effort these bikes provide, which is particularly appealing for occasional or leisure use.
Members: The balanced bike type preference among members indicates their versatility in using the service for diverse activities, reflecting their familiarity and comfort with both bike types.
Service and Marketing Strategies:

Promotional Activities for Casual Riders: Given the strong interest in electric bikes among casual riders, marketing efforts could highlight the benefits of electric bikes, such as easier rides and reduced physical exertion. Promotions could include targeted ads, informational campaigns about electric bike features, and special trial offers.
Service Improvements for Members: For members, ensuring the availability and maintenance of both types of bikes is crucial. Service strategies could focus on regularly servicing both electric and classic bikes, and possibly expanding the fleet based on usage data to meet member preferences.
This analysis of bike type preferences provides valuable insights into the distinct needs and desires of casual and member riders, allowing Cyclistic to tailor its services and marketing efforts to better align with user preferences and enhance overall rider satisfaction.
...
Based on the findings from the data analysis, the following recommendations are proposed to optimize Cyclistic's bike-share system and enhance the user experience for both annual members and casual riders: 
1. Increase maximum ride duration limit for members: Given that casual riders tend to take longer rides than members, increasing the maximum ride duration limit for members could provide more flexibility and accommodate longer trips, especially on weekends when ridership is higher. This adjustment could enhance the user experience for members and encourage casual riders to consider becoming annual members.

2. Increase the number of electric bikes: Casual riders have shown a strong preference for electric bikes, indicating a preference for convenience and ease of use. To cater to this demand, Cyclistic should consider expanding its fleet of electric bikes to ensure availability and meet the needs of casual riders. This strategic investment could cause more casual riders to convert to annual memberships and increase overall ridership.
